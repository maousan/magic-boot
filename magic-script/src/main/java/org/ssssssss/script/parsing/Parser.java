...
public class Parser {
	...
	private VariableDefine parseVarDefine() {
		...
		if ((isMapAccess = stream.match(false, LeftCurly)) || stream.match(false, LeftBracket)) {
			...
			VariableDefine restDefine = null;
			...
			do {
				if (stream.match(Spread, false)) {
					...
					if (hasRest) {
						MagicScriptError.error("解构语法中只能有一个剩余属性", spread.getSpan());
					}
					hasRest = true;
					Token restToken = stream.expect(Identifier);
					String restName = restToken.getSpan().getText();
					checkKeyword(restToken.getSpan());
					boolean isConst = "const".equals(opening.getText());
					VarIndex restVarIndex = forceAdd(restName, isConst);
					defines.add(restName);
					restDefine = new VariableDefine(addSpan(spread.getSpan(), restToken.getSpan()), restVarIndex, null);
				} else {
			...
			VariableDestructuringDefine destructuring = new VariableDestructuringDefine(addSpan(opening, stream.getPrev().getSpan()), tokens.size(), parseExpression(), isMapAccess, restDefine);
