...
public class FunctionCallHandle {
	...
	private static final MethodHandle DESTRUCTURING_REST;
	...
	static {
		try {
			...
			DESTRUCTURING_REST = lookup.findStatic(FunctionCallHandle.class, "destructuring_rest", methodType(Object.class, RuntimeContext.class, Object.class, String.class, Object.class));
	...
	public static CallSite bootstrap(MethodHandles.Lookup caller, String name, MethodType type, int flag) {
		...
		switch (name){
			...
			case "destructuring_rest" : fallback = DESTRUCTURING_REST; break;
		}
		if(fallback != null){
	...
	public static Object destructuring_rest(RuntimeContext runtimeContext, Object target, String type, Object extractedInfo) {
		if (target == null) {
			return null;
		}
		
		if ("object_rest".equals(type)) {
			// 对象剩余属性，extractedInfo 是已提取属性名数组
			Set<String> extractedKeys = new HashSet<>();
			if (extractedInfo instanceof Object[]) {
				Object[] keysArray = (Object[]) extractedInfo;
			...
			if (target instanceof Map) {
				...
				// 当对象解构语法应用于数组时，将数组转换为对象（Map）
				// 符合 JavaScript 行为：数组是特殊的对象，有数字索引属性和 length 属性
				Map<Object, Object> arrayAsObject = new LinkedHashMap<>();

				if (target instanceof List) {
					List<?> list = (List<?>) target;
					for (int i = 0; i < list.size(); i++) {
						arrayAsObject.put(String.valueOf(i), list.get(i));
					}
					arrayAsObject.put("length", list.size());
				} else {
					Object[] array = (Object[]) target;
					for (int i = 0; i < array.length; i++) {
						arrayAsObject.put(String.valueOf(i), array[i]);
					}
					arrayAsObject.put("length", array.length);
				}

				// 排除已提取的属性
				Map<Object, Object> result = new LinkedHashMap<>();
				for (Map.Entry<Object, Object> entry : arrayAsObject.entrySet()) {
					if (entry.getKey() instanceof String) {
						String key = (String) entry.getKey();
						if (!extractedKeys.contains(key)) {
							result.put(key, entry.getValue());
		...
		} else if ("array_rest".equals(type)) {
			// 数组剩余属性，extractedInfo 是已提取的数量
			int extractedCount = 0;
			if (extractedInfo instanceof Integer) {
				extractedCount = (Integer) extractedInfo;
			} else if (extractedInfo instanceof Number) {
				extractedCount = ((Number) extractedInfo).intValue();
			}
			
			if (target instanceof List) {
				List<Object> list = (List<Object>) target;
				if (extractedCount >= list.size()) {
					return new ArrayList<>();
				}
				return new ArrayList<>(list.subList(extractedCount, list.size()));
			} else if (target.getClass().isArray()) {
				Object[] array = (Object[]) target;
				if (extractedCount >= array.length) {
					return new ArrayList<>();
				}
				List<Object> result = new ArrayList<>();
				for (int i = extractedCount; i < array.length; i++) {
					result.add(array[i]);
				}
				return result;
			}
