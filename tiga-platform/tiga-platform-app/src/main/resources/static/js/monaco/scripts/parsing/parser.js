...
define([
	'../editor/java-class.js',
	'./index.js',
	'./ast.js',
	'../editor/request-parameter.js'
	], 
	
function(JavaClass, parsingIndex, parsingAst,RequestParameter) {
    
    // 解构导入
    const {ParseException, Span, TokenType  } = parsingIndex;
	const {
	Assert,
	AsyncCall,
	BinaryOperation,
 ...
 const keywords = ["import", "as", "var", "let", "const", "return", "break", "continue", "if", "for", "in", "new", "true", "false", "null", "else", "try", "catch", "finally", "async", "while", "exit", "and", "or", "throw"/*"assert"*/];
 const linqKeywords = ["from", "join", "left", "group", "by", "as", "having", "and", "or", "in", "where", "on", "limit", "offset"];
...
class Parser {
	...
	parseAccessOrCallOrLiteral(expectRightCurly) {
		let expression;
		if (expectRightCurly && this.stream.match(TokenType.RightCurly, false)) {
			return null;
...
function parseJson(bodyStr, srcObj) {
	srcObj = srcObj || {}
	try {
...
return {parseJson, keywords , linqKeywords , Parser};
});
