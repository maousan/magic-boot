...
import com.ocean.tigaapi.engine.magic.function.context.SqlFunctionExtension;
...
public class MagicEngine {
    ...
    //1、监控与度量 (Metrics)： 它是引擎的“里程碑”。通过它，运维人员可以直观地看到系统自启动以来总共处理了多少次脚本任务。通过 getMetrics() 接口暴露出来后，可以接入 Prometheus 或 Grafana 监控系统，观察 TPS（每秒处理量）的变化。
    //2、健康诊断： 如果发现 executionCount 停止增长，而 Web 请求仍在进来，说明线程池可能发生了严重的死锁或阻塞，它是判断引擎是否“活着”的重要指标。
    //3、灰度对比： 在进行系统升级或脚本迁移时，通过对比不同实例的执行总数，可以确认负载均衡是否均匀分配了任务。
	...
	public MagicEngine() {
    	// 注册自定义持有上下文环境的函数加载器
		MagicScriptEngine.addDefaultImport("sql", new DynamicModuleImport(SqlFunctionExtension.class, context -> {
        ...
        // 绑定缓存大小
        MagicMonitor.bindCacheSize(this.scriptCache);
    ...
    public Object execute(String scriptId, String scriptText, Map<String, Object> params, long timeoutMs) throws Exception {
        if (Utils.isEmpty(scriptText)) return null;
    ...
    private MagicScript getOrCreateScript(String scriptId, String scriptText) {
        ...
        String currentMd5 = Utils.md5(scriptText);
        ...
        if (entry != null && currentMd5.equals(cachedMd5)) {
        	MagicMonitor.recordHit(true); // 【埋点：命中】
            return entry;
        }
        ...
        try {
            ...
            if (entry != null && scriptMd5Cache.get(scriptId).equals(cachedMd5)) {
            	MagicMonitor.recordHit(true); // 【埋点：二次命中】
                return entry;
            }
            log.info("MagicScript 变更，触发重新编译: {}", scriptId);
            MagicMonitor.recordHit(false); // 【埋点：未命中/重编译】
    ...
    private Object submitAndWait(Callable<Object> task, String scriptId, long timeoutMs, long start) throws Exception {
        ...
        try {
        	...
        	// 【计时结束并记录】
        	MagicMonitor.recordExecution(scriptId, System.currentTimeMillis() - start);
            return result;
    ...
    public Map<String, Object> getMetrics() {
       ...
       // metrics.put("executionTotalCount", executionCount.sum());
     ...
     * @param bps     断点行号集合
     */
    public void executeDebug(String sid, String scriptText, Map<String, Object> params, Long timeout, Set<Integer> bps) {
    	// 使用独立的调试线程执行，防止卡死主线程
        executor.submit(() -> {
	    	try {
	            ...
	            // 2. 初始化调试会话
	            MagicDebugManager.initSession(sid, adjustedBps);
	            
	            // 3. 构建调试上下文并注入全局模块与参数
	            MagicDebugContext context = new MagicDebugContext(sid, globalModules);
	            if (params != null) {
	                params.forEach(context::set);
	            }
	            context.set("__scriptId__", sid);
	            ...
	            if (e instanceof InterruptedException || "DEBUG_STOPPED".equals(e.getMessage()) 
	                ...
	                DebugWebSocket.push(sid, "STOPPED", "调试已由用户终止");
	            } else {
	                log.error("调试执行异常 [SID: {}]: ", sid, e);
	                DebugWebSocket.push(sid, "ERROR", e.getMessage());
	            }
	        } finally {
	        	// 清理与该脚本相关的 CalciteEngine 缓存（SQL DSL）
	            CalciteEngine.clearCache(sid);
	            // 必须销毁会话，防止 BlockingQueue 内存泄漏
	            MagicDebugManager.cleanSession(sid);
	        }
        });
    }
