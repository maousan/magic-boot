...
public class GroovyMonitor {
    ...
    private static final Counter cacheHitCounter = Counter.builder("tiga_groovy_cache_hits")
            .description("Total number of script cache hits")
            .register(registry);

    private static final Counter cacheMissCounter = Counter.builder("tiga_groovy_cache_misses")
    ...
    public static void recordExecution(String scriptId, long durationMs) {
        Timer.builder("tiga_groovy_execution")
    ...
    public static void bindCacheSize(Map<?, ?> cache) {
        Gauge.builder("tiga_groovy_cache_size", cache, Map::size)
