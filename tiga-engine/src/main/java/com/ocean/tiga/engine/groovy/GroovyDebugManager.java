...
public class GroovyDebugManager {
    ...
    // 调试监听器：sid -> listener
    private static final Map<String, DebugListener> listeners = new ConcurrentHashMap<>();
     ...
     * 设置模块名称集合
     */
    public static void setModuleNames(Set<String> names) {
        moduleNames = names;
    }
     ...
     * @param sid 调试会话 ID
     * @param bps 断点行号集合
     * @param listener 调试监听器
     */
    public static void initSession(String sid, Set<Integer> bps, DebugListener listener) {
        cleanup(sid);
        status.put(sid, 0);
        breakpointsMap.put(sid, bps != null ? bps : new HashSet<>());
        waiters.put(sid, new Semaphore(0));
        if (listener != null) {
            listeners.put(sid, listener);
        }
    }
    ...
    public static void pushSnapshot(String sid, int line, Map<String, Object> vars, String type) {
        DebugListener listener = listeners.get(sid);
        if (listener != null && "BREAKPOINT_HIT".equals(type)) {
            listener.onBreakpointHit(sid, line, vars);
        }
        // TRACE 类型暂不推送，避免过多通知
    }
